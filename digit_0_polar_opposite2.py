...
# ============= COLOR CODES (Disabled for server compatibility) =============
# Check if we're on a terminal that supports colors
def supports_color():
    """Check if terminal supports color output"""
    # If we're on Windows and using cmd/powershell, colors might not work
...
# ============= AUTHENTICATION HELPERS (Python version) =============
def requests_request(method: str, url: str, headers: dict) -> tuple[int, str]:
    """
    Runs a request using Python's requests library.
    """
    try:
...
def get_authenticated_ws_url(app_id: str, api_token: str, account_type: str = "real") -> str:
    ...
    status, body = requests_request("GET", f"{REST_BASE}/trading/v1/options/accounts", headers)
    if status != 200:
...
class Display:
    @staticmethod
    def clear_screen():
        # Use ANSI clear or just print newlines if colors disabled
        if USE_COLORS:
...
class ZeroXTradeEngine:
    def __init__(self, decimal_places: int, stake: float = 1.0, symbol: str = "R_100", websocket=None, max_trades: int = 0):
        ...
        self.pending_trade_result = None
    ...
    def _place_real_trade(self) -> bool:
        if not self.websocket:
            print(" WARNING: No websocket available for real trading")
            return False
    ...
    def process_tick(self, price: float):
        ...
        if self.state == "IDLE":
            ...
            if digit == '0':
                ...
                x_digit_value = self.x_digit
               
                if self.previous_x is not None and digit == self.previous_x:
                    print(f" SKIPPED: X={digit} (same as previous X={self.previous_x})")
                    self.state = "IDLE"
                    self.x_digit = None
                elif self.is_locked:
...
class ZeroXTradeBot:
    ...
    def display_ui(self):
        ...
        paper_pnl_color = "" if stats['paper_total_pnl'] >= 0 else ""
        real_pnl_color = "" if stats['real_total_pnl'] >= 0 else ""
        ...
        print(f" Status: {lock_status} Loss Streak: {stats['paper_loss_streak']}/2")
        print(f" Real Trades: {stats['real_trade_count']}/{stats['max_real_trades'] if stats['max_real_trades'] > 0 else 'unlimited'}")
        print("=" * 100)
        print("\n PAPER TRADING:")
        print(f" Trades: {stats['paper_trade_count']} | Wins: {stats['paper_wins']} | Losses: {stats['paper_losses']}")
        print(f" Win Rate: {stats['paper_win_rate']:.1f}% | P&L: ${stats['paper_total_pnl']:.2f}")
        print("\n REAL TRADING:")
        print(f" Trades: {stats['real_trade_count']} | Wins: {stats['real_wins']} | Losses: {stats['real_losses']}")
        print(f" Win Rate: {stats['real_win_rate']:.1f}% | P&L: ${stats['real_total_pnl']:.2f}")
        print("\n TOTAL:")
        print(f" Trades: {stats['total_trades']} | Wins: {stats['total_wins']} | Losses: {stats['total_losses']}")
        print(f" Win Rate: {stats['total_win_rate']:.1f}% | P&L: ${stats['total_pnl']:.2f}")
        ...
        if stats['trade_history']:
            ...
            for trade in reversed(stats['trade_history'][-15:]):
                ...
                result = trade['result']
                x_digit = trade['x_digit'] if trade['x_digit'] else "?"
                pnl = f"${trade['pnl']:.2f}"
                print(f" {time_str:>10} | {trade_type:>7} | {x_digit:>7} | {result:>6} | {pnl:>6}")
            print(" " + "-" * 70)
        else:
            print(" No trades yet")
        print("\n Summary:")
